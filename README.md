...
Loyihada Railway uchun ikkita fayl tayyor: `railway.toml` (ishga tushirish
buyrug'ini belgilaydi) va kod allaqachon Railway talabiga moslab yozilgan
(quyida tushuntirilgan). Qadamlar:
...
2. Loyiha **Settings → Variables** bo'limida quyidagilarni qo'shing:
   ...
   `small` o'rniga tavsiya etiladi. `RAILPACK_DEPLOY_APT_PACKAGES=ffmpeg` —
   Railway'ning yangi qurilish tizimi (Railpack) orqali `ffmpeg`ni
   o'rnatadi, bu `pydub` uchun shart.)
...
4. Deploy qiling. `railway.toml` allaqachon `python bot.py` buyrug'ini va
   xatolik bo'lsa qayta ishga tushirish siyosatini belgilab qo'ygan.
...
### Kunlik mashq
"🎯 Kunlik mashq" tugmasi → bot tanlangan tilda jumla aytib beradi (ovozli) →
foydalanuvchi xuddi shunday talaffuz qilib, ovozli xabar yuboradi → Whisper
orqali matnga aylantiriladi → maqsadli matn bilan solishtirilib, foiz
ko'rinishida natija va izoh beriladi. Mashq tugagach, **adminga hisobot**
...
| `handlers_exercises.py` | Kunlik talaffuz mashqi oqimi |
